...
We do "many-at-once" backend work: a batch of independent workstreams that can
run in parallel. The prior setup used Claude Code as a live orchestrator
driving Codex, and it was slow. Root cause, confirmed by profiling: **Claude
was on the execution critical path**. Two ways this happened:
...
Measured on the 2026-07-20→22 sessions: codex workstreams run 7–27 minutes
each (not seconds), and the real tax is the orchestrator staying on-path
around them — in the one batch run, dispatch was serialized over 30 minutes of
brief-writing, every workstream wakeup cost a full max-effort main-loop turn
(79 requests and 33 minutes of model wall during an 80-minute execution
phase), and the session /compact'd twice mid-run. Codex's parallelism never
reaches the wall clock, and progress is invisible to the human, who ends up
polling the orchestrator ("are you done?").
...
wall no matter how many workstreams run.
...
depth rather than speed, and workstream independence — which the whole
pipeline assumes — is produced here, not checked at runtime.

- Decompose the goal into N **independent, self-contained** workstreams.
- **Conflict avoidance is the planner's responsibility.** Partition along file
  boundaries so no two workstreams edit the same file. When overlap is
  genuinely unavoidable, record it in `spec.md` (which workstreams, which
  files) so the merge phase expects those conflicts. There is no runtime
  pre-flight check.
- Write a shared `spec.md` (architecture, constraints, patterns, out-of-scope,
  known overlaps).
- Write `workstreams.txt`: **one workstream per line, each line a pointer, not
  the whole brief.** Example line:
...
- Maintain a **worktree pool** sized to concurrency (see Parameters) so
  parallel workstreams never write the same tree. (Shared-tree parallel
  writers = data race; rejected.) Pool size = concurrency, **not** workstream
  count — a workstream grabs a free worktree, does its work, commits to a
  per-workstream branch, releases the worktree for the next queued workstream.
  Worktrees are created on demand during execution and live only until the
  merge phase completes; nothing persists across runs.
- Run workstreams concurrently (`xargs -P` or equivalent).
- Per workstream: `codex exec "<line>"` → run the per-workstream **check** →
  if green, commit its branch. If red, **bounded retry**: re-invoke Codex in
  the same worktree with the failing check output appended to the prompt, up
  to `retries` times. A workstream that hangs past a per-workstream timeout or
  finishes with no diff counts as red — both happened in the 2026-07-22 run
  (one resume hung, one died at 0s on sandbox scope). On exhaustion, mark the
...
user never asked for, and workstream→integration→session is one merge stage
too many).
...
- After the last merge, run `check` once on the session branch: conflict
  resolutions are code that no workstream gate covered. A RED check restores
  the branch with `git reset --keep` to the pre-merge commit and keeps the
  green workstream branches for autopsy — the session worktree ends exactly
  where it started.
- Delete workstream worktrees and merged workstream branches.

### 4. Review & deliver (Codex, no Claude)

- Local `codex exec review --base <pre-merge>` covers exactly the merged
  workstream delta (not older commits already on the branch); findings land in
  the run logs.
...
- `workstreams` — the work list / spec pointers
- `base`        — the session branch (default: the repo's checked-out branch;
  when given explicitly it must match the checkout)
- `feature`     — run name; namespaces workstream branches and run state (no
  branch is created with this name)
- `check`       — per-workstream verify command (`yarn test`, `cargo test`, …)
- `concurrency` — default = CPU count, overridable
- `retries`     — per-workstream retry budget on a red check (default 2)
- `timeout`     — per-codex-invocation seconds; a workstream past it counts as
  red (default 2400 — the longest measured workstream was 27m)
...
- **Claude Code Workflow with Codex-in-each-agent** — double-wraps every
  workstream (Claude latency + Codex latency ×N). This was the original slow
  design.
...
- **Shared worktree across workstreams** — data race on file writes.
- **One worktree per workstream** — unnecessary; pool sized to concurrency
  suffices.
- **One PR per workstream** — review fragments into N small diffs and the
  integrated result is never reviewed or tested as a whole; the session branch
  takes N separate merge events. One PR from the session branch reviews the
  integrated diff; failed workstreams stay isolated anyway because they are
  excluded at merge.
- **Intermediate feature branch/worktree for integration** — removed
  ...
  stage (workstream → feature → base) and an extra worktree needing deps
