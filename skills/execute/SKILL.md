---
name: "codex:execute"
description: Execute a planned feature as parallel codex tasks off the Claude critical path — convert the session's plan into spec.md+tasks.txt, launch execute.sh (worktree pool, per-task checks, bounded retries, merge onto the session branch, codex review, push), then relay the result. Use when the user wants many-at-once implementation work delegated to codex.
---
...
self-contained** backend tasks codex can execute in parallel. NOT for
...
`docs/`), committed to the session branch so every task worktree carries them.
...
${CLAUDE_PLUGIN_ROOT}/skills/execute/execute.sh \
  --tasks <path>/tasks.txt --feature <run-name> \
...
- Worktree pool sized to concurrency at `../.codex-execute-<feature>/w*`,
  ...
  fully removed after merge. Task branches `tasks/<feature>/<n>`.
...
- Merge: green task branches merge DIRECTLY onto the session branch in the
  session worktree (the tree must still be clean and on the same branch);
...
- Post-merge `check` runs on the session branch. RED restores the branch with
  `git reset --keep` to the pre-merge commit and keeps every green task branch
  for autopsy — the session worktree ends exactly where it started.
- Exit 0 = all green + delivered; 2 = partial (some tasks failed, session
  branch green + delivered); 1 = post-merge check red (branch restored),
  merge blocked (dirty/switched worktree), or push failed.
...
exactly the merged task delta, findings written to
...
PR URL to the user. FAILED tasks are listed in the PR body — offer to re-plan
just those as a new small run (new run name) rather than re-entering the loop
yourself.

## Cleanup

None on success — task worktrees and branches are already gone and the work is
on the session branch. After a red post-merge check, the kept
`tasks/<feature>/<n>` branches and `refs/codex-execute/<feature>/pre-merge`
